...
Mirrors hermes-agent: agent/reasoning_effort.py

The internal effort ladder (``VALID_REASONING_EFFORTS`` plus ``none``) is wider than any
...
# disable can be clamped when a provider publishes it as a level. ``ultra`` is internal
...
def parse_reasoning(value: str | None) -> dict | None:
    """User-facing ``--reasoning`` value -> ``reasoning_config``.

    ``None``/"" = unset (the provider's default applies); "off"/"none"/"false" =
    ``{"enabled": False}``; "on" = enabled with no explicit effort; a ladder level =
    that effort. Unknown names pass through: bespoke providers may accept them.
    """
