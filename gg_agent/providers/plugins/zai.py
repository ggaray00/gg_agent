...
GLM-4.5+ defaults to thinking ON, so ``reasoning_config`` becomes
...
def glm_5_2_reasoning_effort(reasoning_config: dict | None, *, model: str | None = None) -> str | None:
    """Effort -> GLM vocabulary (5.2: high/max; 5.3: low..max). Below-floor efforts
    clamp to the floor; disabled/unset leaves the server default."""
...
class ZaiProfile(ProviderProfile):
    def build_api_kwargs_extras(self, *, reasoning_config: dict | None = None,
        ...
        # Only when the user expressed a preference (server default = enabled).
        if isinstance(reasoning_config, dict):
            enabled = reasoning_config.get("enabled") is not False
            extra_body["thinking"] = {"type": "enabled" if enabled else "disabled"}
        if is_5_2:
            effort = glm_5_2_reasoning_effort(reasoning_config, model=model)
            if effort is not None:
                top_level["reasoning_effort"] = effort
        return extra_body, top_level


register_provider(ZaiProfile(
    name="zai", aliases=("glm", "z-ai", "z.ai", "zhipu"), display_name="Z.AI (GLM)",
    description="Z.AI / GLM — Zhipu AI models", signup_url="https://z.ai/",
    env_vars=("GLM_API_KEY", "ZAI_API_KEY", "Z_AI_API_KEY"), base_url="https://api.z.ai/api/paas/v4",
