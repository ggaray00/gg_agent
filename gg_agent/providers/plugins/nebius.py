"""Nebius Token Factory provider profile: top-level reasoning_effort."""
...
_REASONING_MARKERS = ("deepseek-r1", "deepseek-v4", "deepseek-reasoner", "gpt-oss", "glm-5", "kimi-k2",
                      "minimax-m2", "qwen3")
...
class NebiusTokenFactoryProfile(ProviderProfile):
    def build_api_kwargs_extras(self, *, reasoning_config: dict | None = None, model: str | None = None,
        ...
        name = (model or "").strip().rsplit("/", 1)[-1].lower()
        if not supports_reasoning and not any(marker in name for marker in _REASONING_MARKERS):
            return {}, {}
...
register_provider(NebiusTokenFactoryProfile(
